...
- **`05-requirements.md` was regenerated at a finer grain, after the fact.** The original
  version collapsed most of the Basic Path into two requirements; per the fix described in
  `TODO.md`, it now has one requirement per distinct effect (check window, mark claimed,
  notify lister, remove from board), six instead of four.
