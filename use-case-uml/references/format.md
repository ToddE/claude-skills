...
followed by a numbered list of the diverging steps, ending with:
`Use case continues at Basic Path #<N>.`

**Exception Paths** — same pattern as Alternate Paths, but for error or failure branches
instead of valid variations. Use `TBD` for steps not yet worked out rather than leaving the
item off.
